...
import torch
import trimesh
from airobot import log_critical, log_debug, log_info, log_warn, set_log_level
...
def main(args):
    ...
    hydra.initialize(config_path="../configs", job_name="eval_bottle_rndf")
    ...
    hydra_cfg = hydra.compose(
        config_name="eval_full_bottle_place",
        overrides=["pose_dist={:s}".format(pose_dist)],
        return_hydra_config=True
    )
    hydra.core.hydra_config.HydraConfig.instance().set_config(hydra_cfg)
    ...
    for iteration in range(args.start_iteration, args.num_iterations):
        ...
        print(parent_pcd.shape, child_pcd.shape)

        tmp_obj_points = np.concatenate([child_pcd, parent_pcd])
        tmp_obj_classes = np.concatenate([np.zeros(len(child_pcd)), np.ones(len(parent_pcd))])

        points_mug_raw, points_rack_raw = load_data_raw(
            num_points=1024,
            clouds=tmp_obj_points,
            classes=tmp_obj_classes,
            action_class=0,
            anchor_class=1,
        )
        ...
        relative_trans = util.matrix_from_pose(transform_rack_relative_pose)
